...
class _CompactCard(_GlassCard):
    def __init__(self):
        super().__init__()
        self.setFixedSize(215, 172)
        lay = QVBoxLayout(self)
        lay.setContentsMargins(14, 14, 14, 12)
        lay.setSpacing(6)

        # Ligne MMR : [icône rang 20px] [label MMR] → [delta] [mmr value]
        ...
        mmr_lbl_w.setStyleSheet(
            f"color:{C_MUTE};font-size:9px;letter-spacing:1.2px;background:transparent;")

        self.v_delta = QLabel("")
        self.v_delta.setStyleSheet(
            f"color:{C_GREEN};font-size:11px;font-weight:700;background:transparent;")
        self.v_mmr = QLabel("--")
        self.v_mmr.setStyleSheet(
            f"color:{C_GOLD};font-size:18px;font-weight:700;background:transparent;")

        mmr_row.addWidget(self._rank_icon)
        mmr_row.addWidget(mmr_lbl_w)
        mmr_row.addStretch()
        mmr_row.addWidget(self.v_delta)
        ...
        def row(lbl_txt, val_color, size=16):
            ...
            v = QLabel("--")
            v.setStyleSheet(
                f"color:{val_color};font-size:{size}px;font-weight:700;background:transparent;")
    ...
    def update_stats(self, d, mmr_mode="both"):
        ...
        if mmr_mode == "delta":
            ...
            if chg:
                ...
                self.v_delta.setStyleSheet(
                    f"color:{clr};font-size:18px;font-weight:700;background:transparent;")
            else:
                self.v_delta.setText("--")
                self.v_delta.setStyleSheet(
                    f"color:{C_MUTE};font-size:18px;font-weight:700;background:transparent;")
        elif mmr_mode == "mmr":
            ...
            if chg and mmr:
                ...
                self.v_delta.setText(f"{sign}{chg}")
                self.v_delta.setStyleSheet(
                    f"color:{clr};font-size:11px;font-weight:700;background:transparent;")
            else:
                self.v_delta.setText("")
