...
class _ScoreboardCard(QWidget):
    ...
    def _build(self):
        ...
        lbl_b = QLabel("WIN")
        lbl_b.setStyleSheet(
            "color:#3AE08A;font-size:8px;font-weight:700;letter-spacing:2px;background:transparent;")
        val_b = QLabel("0")
        val_b.setStyleSheet(
            "color:#3AE08A;font-size:16px;font-weight:900;background:transparent;")
        ...
        vs.setFixedWidth(36)
        vs.setStyleSheet(
            "color:rgba(200,210,230,0.4);font-size:9px;font-weight:700;letter-spacing:2px;background:transparent;")
        ...
        val_o = QLabel("0")
        val_o.setStyleSheet(
            "color:#ff3d57;font-size:16px;font-weight:900;background:transparent;")
        lbl_o = QLabel("LOSS")
        lbl_o.setStyleSheet(
            "color:#ff3d57;font-size:8px;font-weight:700;letter-spacing:2px;background:transparent;")
        ...
        # Bas : MMR (avec rang inline), DELTA, STREAK
        bot = QWidget()
        ...
        def mini_block(key, label, color, with_rank=False):
            w = QWidget()
        ...
        bot_lay.addWidget(mini_block("mmr",   "MMR",    "#00cfff", with_rank=True))
    ...
    def paintEvent(self, event):
        ...
        gb.setColorAt(0.0, QColor(10, 60, 35, 210))
        gb.setColorAt(1.0, QColor(10, 50, 28, 180))
        p.setBrush(QBrush(gb))
        p.drawRoundedRect(QRectF(0, 0, half + 8, h), 8, 8)

        go = QLinearGradient(half, 0, w, 0)
        go.setColorAt(0.0, QColor(80, 10, 20, 180))
        go.setColorAt(1.0, QColor(100, 10, 22, 210))
    ...
    def update_stats(self, d, mmr_mode="both"):
        ...
        if chg and mmr_mode != "mmr":
            ...
            self._vals["delta"].setStyleSheet(
                f"color:{clr};font-size:15px;font-weight:800;background:transparent;")
        else:
            self._vals["delta"].setText("--" if mmr_mode != "mmr" else "")
            self._vals["delta"].setStyleSheet(
                "color:#555;font-size:15px;font-weight:800;background:transparent;")
        ...
        if sv > 0:
            ...
            self._vals["stk"].setStyleSheet(
                "color:#00e676;font-size:15px;font-weight:800;background:transparent;")
