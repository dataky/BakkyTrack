...
class _SidebarCard(QWidget):
    ...
    def _build(self):
        ...
        def block(key, label, val_color, with_rank=False):
            w = QWidget()
        ...
        # ── Blocs stats (MMR avec rang inline) ────────────────────────────
        root.addWidget(block("mmr",    "MMR",    "#00cfff", with_rank=True))
        root.addWidget(block("delta",  "DELTA",  "#aaaaaa"))
        root.addWidget(block("wins",   "WINS",   "#00e676"))
        root.addWidget(block("losses", "LOSSES", "#ff3d57"))
        root.addWidget(block("stk",    "STREAK", "#00e676"))
        root.addStretch()
    ...
    def paintEvent(self, event):
        ...
        colors = [
            ...
            QColor(0, 60, 30, 50),   # wins — vert
            QColor(60, 10, 15, 50),  # losses — rouge
            QColor(0, 60, 30, 40),   # stk — vert
        ]
        block_h = 46
        y_start = 30  # header 26 + winrate 4
