...
class StatsTab(QWidget):
    ...
    def _build(self):
        ...
        # ── Boutons +/− pour ajustement manuel (tout sur une ligne) ──────
        adj_row = QHBoxLayout()
        ...
        rmw = btn("− Victoire", bg="rgba(255,61,87,0.08)", fg=C_RED, size=9)
        rmw.setFixedWidth(90)
        rmw.clicked.connect(lambda: self.app.remove("win"))
        rml = btn("− Défaite", bg="rgba(0,230,118,0.08)", fg=C_GREEN, size=9)
        rml.setFixedWidth(90)
        rml.clicked.connect(lambda: self.app.remove("loss"))
        ...
        adj_row.addWidget(rmw)
        adj_row.addSpacing(4)
        adj_row.addWidget(rml)
        adj_row.addSpacing(4)
